...
//#pragma HLS pipeline
    		for (j = 0; j < N; ++j) {
#pragma HLS unroll
    			lstm_state[j][i] = HLS_REG<ap_int<16>, 1>(HLS_REG<ap_int<16>, 0>(sampleinput_16b) * weight_l_pre[i][j]) >> 12;
    		}
    	}
...
#pragma HLS pipeline II=1
    			for (j = 0; j < N; ++j) {
#pragma HLS unroll
    				lstm_state[j][0] += (HLS_REG<ap_int<16>, 3>(HLS_REG<ap_int<16>, 2>(h_state[k]) * weight_l_mid_0[k][j]) >> 12);
    				lstm_state[j][1] += (HLS_REG<ap_int<16>, 3>(HLS_REG<ap_int<16>, 2>(h_state[k]) * weight_l_mid_1[k][j]) >> 12);
    				lstm_state[j][2] += (HLS_REG<ap_int<16>, 3>(HLS_REG<ap_int<16>, 2>(h_state[k]) * weight_l_mid_2[k][j]) >> 12);
    				lstm_state[j][3] += (HLS_REG<ap_int<16>, 3>(HLS_REG<ap_int<16>, 2>(h_state[k]) * weight_l_mid_3[k][j]) >> 12);
    			}
    	}
